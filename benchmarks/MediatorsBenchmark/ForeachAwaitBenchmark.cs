...
public class ForeachAwaitBenchmark
{
    ...
    [Benchmark]
    public async Task MediatR_Publish_ForeachAwait_notification_has_single_handler()
    {
        await _mediatR.Publish(new MediatRNotificationWithSingleHandler(), CancellationToken.None);
    }

    [Benchmark]
    public async Task NimbleMediator_Publish_ForeachAwait_notification_has_single_handler()
    {
        await _nimbleMediator.PublishAsync(new NimbleMediatorNotificationWithSingleHandler(), CancellationToken.None);
    }

    [Benchmark]
    public async Task MediatR_Publish_ForeachAwait_notification_has_multiple_handlers()
    {
        await _mediatR.Publish(new MediatRNotificationWithMultipleHandlers(), CancellationToken.None);
    }

    [Benchmark]
    public async Task NimbleMediator_Publish_ForeachAwait_notification_has_multiple_handlers()
    {
